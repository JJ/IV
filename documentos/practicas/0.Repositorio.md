...
* Los mensajes de *commit* son predicados (*hace* tal cosa, *modifica*
  tal otra), *informativos* (que diga efectivamente por qué se ha
  hecho algo, no simplemente "cambia fichero" o "commit"), se deben
  referir a un issue (si no lo cierran, simplemente puede poner el #
  del mismo, por ejemplo, `relacionado con #1`), y con una estructura
  del tipo "primera línea corta, dos retornos de carro, descripción
  más extensa". Por ejemplo:
```
Añade descripción mensajes de commit

Con las características que deben tener los mismos para que el trabajo
en grupo sea fluido
```
