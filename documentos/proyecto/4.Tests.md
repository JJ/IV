...
* **Herramientas CLI para ejecutar los tests**, que para aumentar la confusión,
  ...
  de cobertura (que analizan qué sentencias y ramas se han usado efectivamente
  en los tests). Hay lenguajes o marcos de programación, como Go o `deno`, que
  ya incluyen una suborden `test` que realiza esta labor; en esto casos no habrá
  que establecer, como es natural, un criterio para elegirlo, porque la mejor
  práctica es usar la que ya hay, pero si habrá que dejar claro en la
  documentación del objetivo que este es el caso y que se va a hacer de esa
  forma.
