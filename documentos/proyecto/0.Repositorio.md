# Objetivo 0: Mecánica de la asignatura + desarrollo de un proyecto
...
Hago énfasis en *en resto de la asignatura*. *Realmente* tendrás que intentar
desarrollar un producto que resuelva parcialmente el problema que has planteado
aquí. En el [siguiente objetivo](1.Infraestructura.md) tendrás que plantar los
hitos de desarrollo del proyecto y plasmar las necesidades del usuario, y en el
[segundo objetivo](2.Entidad.md) comenzar a modelizar el problema de forma que
...
En este objetivo cero del proyecto se trata de poner a punto las herramientas
...
servidor que se pueda desplegar en la nube, es decir, tiene que tener descrita
correctamente una **lógica de negocio**, que es el código propio que resuelve el
problema.
...
complicadas y no van a incluir ningún tipo de modelo. Lo esencial siempre es que
la aplicación incluya un valor añadido en forma de lógica de negocio, y que se
describa correctamente el problema a solucionar de forma que haga falta tal
...
La lógica de negocio será testeada más adelante, a partir del objetivo 3. Por
eso es esencial que sea no trivial, para que los tests efectivamente comprueben
que se está resolviendo el problema correctamente. Así mismo, los tests deben
servir para validar que se ha resuelto el problema a través del programa, por lo
que el problema debe ser formulado de forma que efectivamente, el procesamiento
de la información que lleva a cabo el programa se pueda validar como una
...
proyecto donde uno tiene algún interés, y puede identificar claramente cuales
son los retos y qué es lo que funciona y aporta valor al cliente (que puede ser
uno mismo). La metodología denominada [*design
...
Un problema adicional es encontrar proyectos en los cuales existan ya datos. No
va a ser suficientemente empático un proyecto que requiera del usuario
...
cercano, no vas a poder validar la solución.
...
2. En general, va a ser necesario que entiendas el mecanismo de la
   ...
   sesión](https://github.com/JJ/IV-22-23/blob/master/sesiones/semana-01.md)
...
* Usar o bien el sistema de control de versiones que se incluya en el entorno
  ...
  Emacs, VSCode, o algún editor específico para el lenguaje de programación que
  se vaya a usar. Suelen tener un sistema de control de
...
aplicación irá evolucionando con los diferentes objetivos.
...
[al fichero de entrega del proyecto](https://github.com/JJ/IV-22-23/blob/master/proyectos/objetivo-0.md),
...
**Nota**: puedes entregar el objetivo todas las veces que necesites, siguiendo
los consejos del profesor. No trates de *poner lo que pide* en la primera
ocasión, ni en la segunda. Trata de entender qué es lo que tienes que aprender
en cada objetivo, y transmítelo en lo que entregues. Lo importante siempre es lo
que aprendas, no lo que *pongas* o *escribas*. Esto se aplica a este objetivo, y
por supuesto al resto de la asignatura.
...
según vayas comprobando que la respuesta es cierta. Ayudará si pones alguna
explicación de por qué la respuesta es afirmativa.
...
Los objetivos no tienen fecha de entrega. Sin embargo, hay que superar un número
mínimo de objetivos para aprobar la asignatura. Podemos dar, por tanto, medidas
estadísticas a partir del desempeño en años anteriores, para que los estudiantes
se organicen.

* En el curso 2021-22, el 50% de los estudiantes entregó este objetivo [en **5**
  días](https://jj.github.io/IV/calendarios/0-la-mitad.ics),
  el 75% en [**10**
  días](https://jj.github.io/IV/calendarios/0-tres-cuartos.ics).
* Ningún estudiante que aprobó en la convocatoria ordinaria tardó más de 5
  semanas en superar este objetivo. A partir de la 5ª semana del comienzo, sería
  mejor que planificaras el resto de la asignatura para la convocatoria
  extraordinaria.
...
siguiente](1.Infraestructura.md) objetivo. Puedes tener todos los PRs que desees
