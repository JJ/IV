...
2. Tratar de crear issues "como programador" asignados a esas historias de
   usuario, también organizadas de forma lógica, y *que representen un
   problema*. Los issues son siempre *problemas*, no tareas o soluciones al
   mismo. Plantean un problema que se resolverá iterativamente en los
   comentarios al mismo y eventualmente en el código; en este caso la solución
   se planteará directamente en el código, y en ausencia de tests automáticos,
   será el *product manager* quien decida si esa solución es válida en función
   de que sea o no una modelización suficiente y mínima del problema, y que sea
   útil para continuar el desarrollo.
