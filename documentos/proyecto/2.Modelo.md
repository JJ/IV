...
1](1.Planificacion) y partir de las mismas para poder resolver el problema.
...
Se tendrá que haber avanzado en el [objetivo 1](1.Planificacion) hasta
...
1](1.Planificacion). El desarrollo ágil está centrado en el cliente, y el
...
Las historias de usuario se implementan en issues específicos, identificados
como tales (mediante una etiqueta y el título del issue), en el repositorio de
...
### Los issues *son enunciados de problemas*

Si piensas "voy a escribir este código" y a continuación pones como
issue "Hay que escribir este código", ¿para qué te sirve un issue
salvo para añadir burocracia inútil e improductiva?

Los *issues*, que se empezarán a usar de forma extensiva en este
objetivo, deben desgranar un problema que parta siempre de la historia
de usuario, simplificándolo o abstrayéndolo de forma que te puedas
acercar a su solución mediante código. Un issue que te permita
entender mejor el dominio del problema, las interacciones entre
diferentes entidades y objetos valor y cómo se comporta el mismo será
útil. No solo por el hecho de que la ingeniería es solución de
problemas, no escritura de código, sino porque todo issue debe dar
lugar a código que podamos probar que resuelve el problema. Un issue
que enuncia un problema claramente da lugar a código y un test
correspondiente que comprueba sin lugar a dudas que ese código es de
calidad y efectivamente aporta valor al cliente. Si es una orden de
trabajo de algo que ya pensabas escribir, es imposible saber si aporta
valor al cliente o no, porque no tiene ninguna relación ni con las
historias de usuario ni con el dominio del problema, solo con tu
propio modelo interno que no es testeable.
...
La secuencia que se menciona arriba es importante: primero los issues,
luego el código. Si los issues son incorrectos y no hacen un análisis
adecuado del dominio del problema, el código va a ser incorrecto y no
va a ser útil para fases posteriores, en concreto el [objetivo
4](4.Tests). Por eso conviene que el propietario/a del producto
(del repositorio) examine *primero* los issues y los discuta y
perfeccione, antes de pasar a hacer y comentar el código en el PR. *No
vale cualquier issue ni cualquier código* y la parte más importante es
el modelado del problema que es el que se plasma en los issues. De la
misma forma, conviene no pasar a escribir código hasta que todos los
issues estén bien perfilados, modelando el problema en capas de
abstracción crecientes, y con mucha claridad en todas las relaciones
entre las entidades y los objetos valor.
...
> El 50% de los estudiantes en cursos pasados tardó 23 días o menos en hacer la
...
request* **tiene* que incluir la cadena `[IV-25-26]` para que puedan filtrar
...
> Puedes copiar (al mismo lugar) y adaptar el [workflow
> `lint-yaml.yml`](https://github.com/JJ/IV/blob/master/.github/workflows/lint-yaml.yml)
> para comprobar la corrección de este `IV.yaml` y otros ficheros YAML
> que vayas añadiendo.
...
1](1.Planificacion) *deben ser guía suficiente* para los dos retos
